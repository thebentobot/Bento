...
export class ShardUtils {
	...
	public static async recommendedShardCount(token: string, serversPerShard: number): Promise<number> {
	...
	public static async serverCount(shardInterface: ShardingManager | ShardClientUtil): Promise<number> {
		const shardGuildCounts = (await shardInterface.fetchClientValues(`guilds.cache.size`)) as number[];

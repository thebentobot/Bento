import { SelectMenuInteraction, Message, MessageActionRow, MessageSelectMenu, EmbedAuthorData, MessageEmbed, MessageSelectOptionData, EmbedFooterData } from "discord.js";
...
categoryMap.set(`selectMenu_helpCMD_category_admin`, {commandCategorySearch: `admin`, categoryEmbed: `Admin`, description: `Commands only for the administration of the server.\nVarious feature toggles and settings such as welcome messages, log channels etc..`});
categoryMap.set(`selectMenu_helpCMD_category_features`, {commandCategorySearch: `features`, categoryEmbed: `Features`, description: `Various features such as lastfm, gfycat, checking the weather, creating tags etc.`});
categoryMap.set(`selectMenu_helpCMD_category_info`, {commandCategorySearch: `info`, categoryEmbed: `Info`, description: `General information about users and servers.\nAvatar and Banner commands, emotes etc.`});
categoryMap.set(`selectMenu_helpCMD_category_moderation`, {commandCategorySearch: `moderation`, categoryEmbed: `Moderation`, description: `Commands only for moderation of the server.\nKick, ban, mute, case commands etc.`});
categoryMap.set(`selectMenu_helpCMD_category_user`, {commandCategorySearch: `user`, categoryEmbed: `User`, description: `User related commands such as rank profile and its customisation, leaderboards, reminders, bento etc.`});
...
export class HelpSelectMenu implements SelectMenu {
	...
	public async execute(intr: SelectMenuInteraction, _msg: Message, _data: EventData): Promise<void> {
		if (intr.customId === this.ids[0]) {
			...
			initialCategoryMenu.map(category => categorySelectMenu.addOptions({
				...category,
				default: categorySelectMenuValue === category.value ? true : false
			}));
			const categoryRow = new MessageActionRow()
				.addComponents(categorySelectMenu.setCustomId(`selectMenu_helpCMD_category_initial`));
			...
			getCommands.map(command => commandsSelectMenu.addOptions({
				label: command.name as string,
				description: command.slashDescription as string,
				value: command.name as string,
			}));
			const commandRow = new MessageActionRow()
				.addComponents(commandsSelectMenu.setCustomId(categorySelectMenuValue).setPlaceholder(`Pick a command`));
			...
			const authorData: EmbedAuthorData = {
				name: getCommand.commandType === CommandType.Both ? `💬  Message command | / Slash command` : (getCommand.commandType === CommandType.MessageCommand ? `💬 Message command` : `**/** Slash command`)
			};
			...
			const usageText = getCommand.commandType === CommandType.Both ? (`${guildData?.prefix}` + getCommand.usage) : (getCommand.commandType === CommandType.MessageCommand ? (`${guildData?.prefix}` + getCommand.usage) : (getCommand.usage));
			const embed = new MessageEmbed()
				.setAuthor(authorData)
				.setTitle(stylingUtils.capitalizeFirstCharacter(getCommand.name))
				.setDescription(`${getCommand.aliases ? `${getCommand.aliases.length > 1 ? `**Aliases:** ${getCommand.aliases.join(`, `)}\n` : `**Alias:** ${getCommand.aliases[0]}`}\n` : ``}${getCommand.description}\n\n**Usage**\n${usageText}\n\n[Click here to check the command on the website](${getCommand.website})`)
			...
			const commandRow = new MessageActionRow()
				.addComponents(commandsSelectMenu.setCustomId(intr.customId));
			await InteractionUtils.editReply(intr, {embeds: [embed], components: [categoryRow, commandRow]});
			return;
		}
	}
